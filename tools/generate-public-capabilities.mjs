...
export const REPOSITORY = 'mike-axiom-mir/axm-EchoWorld';
...
const PATTERN_PROVENANCE = Object.freeze({
  adapted_from_repository: 'mike-axiom-mir/axm-ignition-fabric',
  adapted_from_ref: '4320b5749980639273ec791c8dde90f880f9f0bf',
  adapted_paths: [
...
function readMarker(root) {
  ...
  if (marker.display_name !== 'AXM EchoWorld') throw new Error('public discovery display name drift');
  return marker;
}
...
function assertCapabilityBoundary(packageDocument) {
  ...
  if (!runtime || runtime.kind !== 'node' || runtime.minimumVersion !== '20') throw new Error('unexpected runtime boundary');
  ...
  if (capability.entrypoints?.library !== '.' || capability.entrypoints?.command !== 'echoworld-replay') {
    throw new Error('unexpected package entrypoint boundary');
  }
  if (capability.entrypoints?.discoveryCommand !== 'echoworld-replay describe') {
    throw new Error('unexpected discovery command');
  }
  if (capability.contracts?.input !== 'axm.echoworld.event-stream/v0.01') throw new Error('unexpected input contract');
  if (capability.contracts?.receipt !== 'axm.echoworld.portable-replay/v0.01') throw new Error('unexpected replay receipt contract');
  if (capability.contracts?.verification !== 'axm.echoworld.portable-replay-verification/v0.01') {
    throw new Error('unexpected verification contract');
  }
...
export async function buildArtifacts(root = process.cwd()) {
  ...
  const executableDescriptor = await loadExecutableDescriptor(root, descriptorSource);
